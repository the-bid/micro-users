...
describe('Date', () => {
  describe('parseValue', () => {
    test('parseValue takes in a date string and outputs a millisecond number', () => {
      const result = Date.parseValue(date('YYYY-MM-DDTHH:mm:ss.SSS') + 'Z')
      expect(result).toEqual(expect.any(Number))
    })
  })
  describe('serialize', () => {
    const result = Date.serialize(date('YYYY-MM-DDTHH:mm:ss.SSS') + 'Z')
    expect(result).toEqual(expect.any(Number))
  })
  describe('parseLiteral', () => {
    test('returns null if ast is a string', () => {
      const result = Date.parseLiteral(astFromValue(date('YYYY-MM-DDTHH:mm:ss.SSS') + 'Z', GraphQLString))
